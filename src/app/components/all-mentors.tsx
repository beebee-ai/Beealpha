           ...
           <div className={`bg-white/95 backdrop-blur-sm rounded-xl p-6 sm:p-8`}>
             <div className="flex flex-col md:flex-row gap-8 items-start">
                ...
                <div className="flex-1 min-w-0">
                  <h3 className={`text-2xl font-bold mb-2 ${isExpert ? 'text-[#0c2e4e]' : 'text-[#2c1810]'}`}>
                    {mentor.name}
                  </h3>
                  <p className={`font-bold mb-4 text-sm sm:text-base uppercase tracking-wide ${isExpert ? 'text-[#00f2fe]/80 text-[#0c2e4e]' : 'text-[#FF9D42]'}`}>
                     {title}
                  </p>
          ...
          <div className="p-8 sm:p-10 bg-white border-t border-gray-100">
             ...
             {quote && (
               <div className="mt-10 relative pl-8 py-2">
                 <Quote className={`absolute top-0 left-0 w-6 h-6 ${isExpert ? "text-blue-400" : "text-orange-400"} opacity-50`} />
                 ...
                 {realProjectMentors.map((mentor) => (
                   <li key={mentor.id}>
                     <button
                       ...
                       className="block w-full text-left text-sm text-gray-600 hover:text-[#FF9D42] transition-colors py-1"
                     >
                       {mentor.name}
                     </button>
                   </li>
                 ...
                 {realProfessionalLecturers.map((mentor) => (
                   <li key={mentor.id}>
                     <button
                       ...
                       className="block w-full text-left text-sm text-gray-600 hover:text-[#4facfe] transition-colors py-1"
                     >
                       {mentor.name}
                     </button>
                   </li>
        ...
        <div className="flex-1 lg:ml-64 p-4 sm:p-8 lg:p-16">
          <div className="max-w-4xl mx-auto space-y-20">
            ...
            <section id="professional-mentors">
              ...
              <div className="space-y-12">
                {realProfessionalLecturers.map((mentor) => 
                  renderMentorCard(mentor, "from-[#4facfe]", "to-[#00f2fe]", true)
                )}
              </div>
            </section>
